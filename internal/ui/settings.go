...
var sectionNames = []string{"General", "GitLab servers", "Groups & roots", "Security"}
...
func (s *settingsView) show(section int) {
	...
	case sectionServers:
		s.content.SwitchToPage("servers")
	case sectionGroups:
...
func (s *settingsView) updateFooter() {
	...
	if !s.list.HasFocus() {
		...
		case sectionGroups:
			keys = "space  scope   ·   d  root directory   ·   r  reload groups   ·   p m  refresh indexes   ·   Esc  back"
		case sectionSecurity:
...
func (s *settingsView) newServersTable() *tview.Table {
	...
	t.SetInputCapture(func(ev *tcell.EventKey) *tcell.EventKey {
		...
		case tcell.KeyEnter:
			if inst != nil {
				s.showServerForm(inst)
			}
			return nil
		case tcell.KeyRune:
			switch ev.Rune() {
			case 'a':
				s.showServerForm(nil)
				return nil
			case 'e':
				if inst != nil {
					s.showServerForm(inst)
...
func (s *settingsView) fillServers() {
	t := s.servers
	t.Clear()
	...
	if len(s.app.cfg.Instances) == 0 {
		t.SetCell(1, 1, tview.NewTableCell("No servers yet - press a to add one").
			SetTextColor(colWarn).SetSelectable(false))
		return
	}
	for i, inst := range s.app.cfg.Instances {
		...
		mark := tview.NewTableCell(" " + tag(colAccent) + "●" + tagEnd).SetReference(i)
		t.SetCell(i+1, 0, mark)
		t.SetCell(i+1, 1, tview.NewTableCell(inst.Label()).SetTextColor(colText))
		t.SetCell(i+1, 2, tview.NewTableCell(inst.URL).SetTextColor(colMuted))
...
func (s *settingsView) selectedInstance() *config.Instance {
	row, _ := s.servers.GetSelection()
	if row < 1 || row >= s.servers.GetRowCount() {
		return nil
	}
	cell := s.servers.GetCell(row, 0)
	if cell == nil {
		return nil
	}
	i, ok := cell.GetReference().(int)
	if !ok || i >= len(s.app.cfg.Instances) {
		return nil
	}
	return &s.app.cfg.Instances[i]
}

// showServerForm adds a server, or edits the one passed in.
func (s *settingsView) showServerForm(inst *config.Instance) {
	a := s.app
	adding := inst == nil
	title := "Edit server"
	...
	form.AddButton("Save", func() {
		...
		url := strings.TrimRight(strings.TrimSpace(form.GetFormItem(1).(*tview.InputField).GetText()), "/")
		root := strings.TrimSpace(form.GetFormItem(2).(*tview.InputField).GetText())
		token := form.GetFormItem(3).(*tview.InputField).GetText()
		...
		target := inst
		if adding {
			target = a.cfg.AddInstance(config.Instance{Name: name, URL: url, RootDir: root})
		} else {
...
func (s *settingsView) showTokenForm(inst *config.Instance) {
	...
	form.AddTextView("", "A personal access token with the api scope.\n"+
...
func (s *settingsView) verifyInstance(inst config.Instance) {
	...
	a.runTask("Verifying "+inst.Label(), func(log func(string)) (string, error) {
		...
		defer cancel()
		log("GET " + inst.URL + "/api/v4/user")
		user, err := client.CurrentUser(ctx)
		if err != nil {
			return "", err
		}
		log(fmt.Sprintf("The token works: %s (%s)", user.Name, user.Username))
		a.tv.QueueUpdateDraw(func() { a.note(inst.Label() + ": signed in as " + user.Username) })
		return "", nil
	})
}
...
func (s *settingsView) confirmRemoveInstance(inst config.Instance) {
	...
	body := fmt.Sprintf("Remove [::b]%s[::-] from unagit?\n\n%s\n\nIts token is deleted and its groups are forgotten.\nNothing already cloned to disk is touched.",
		inst.Label(), inst.URL)
	a.confirm("Remove server", body, nil, func() {
...
func (s *settingsView) groupsOf(instanceID string) []gitlab.Group {
...
func (s *settingsView) labelGroup(node *tview.TreeNode, instanceID string, g gitlab.Group) {
...
func (s *settingsView) cycleGroup(node *tview.TreeNode, instanceID string, g gitlab.Group) {
	inst := s.app.cfg.Instance(instanceID)
	if inst == nil {
		return
	}
	scope := inst.CycleGroup(config.Group{ID: g.ID, FullPath: g.FullPath, Name: g.Name})
